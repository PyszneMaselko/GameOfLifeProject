...
class NewGameWindow(ctk.CTkToplevel):
    ...
    def choose_bg_color(self):
        ...
        if color[1] is not None:
            ...
            self.bg_color_button.configure(text=f"Kolor tła: {self.bg_color}")

    def choose_draw_color(self):
        color = colorchooser.askcolor(title="Wybierz kolor rysowania")
        if color[1] is not None:
            self.draw_color = color[1]
            self.draw_color_button.configure(text=f"Kolor rysowania: {self.draw_color}")
    ...
    def start(self):
        try:
            ...
            self.destroy()
            # Przekazujemy callbackowi także kolory
